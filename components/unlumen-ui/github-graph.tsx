...
> = {
  github: [
    "var(--github-graph-level-0)",
    "var(--github-graph-level-1)",
    "var(--github-graph-level-2)",
    "var(--github-graph-level-3)",
    "var(--github-graph-level-4)",
  ],
...
function selectRecentContributions(
  ...
  // Pin both ends of the calendar window, even if the public API lags a day.
  for (const date of [start, today]) {
    if (!recent.some((contribution) => contribution.date === isoDate(date))) {
      recent.push({ date: isoDate(date), count: 0, level: 0 });
    }
  }
